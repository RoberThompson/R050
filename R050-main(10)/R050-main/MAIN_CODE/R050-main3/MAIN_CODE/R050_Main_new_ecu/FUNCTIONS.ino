...
void steamGenPID2() {
  ...
  if (blwrOverride) {
    ...
    //BLOWER_SPEED_OFFSET = STEAM_GEN_PID2.calculate(PT304_SP_STEAMGEN , PT304_TWV308_INPUT_PRESSURE);

    cmdSpeedBlower = STEAM_GEN_PID2.calculate(PT304_SP_STEAMGEN , PT304_TWV308_INPUT_PRESSURE);
...
void superheatTest() {
  ...
  if (wpOverride) {
    ...
    if (TT306_EJECTOR_STEAM_IN > 200 || TT303_HX504_STEAM_OUT > 200) {
      ...
      modbusTCPClient1.holdingRegisterWrite(41, cmdSpeedWp);//write channel 1 (WP_Speed)
      // modbusTCPClient1.holdingRegisterWrite(43,FCV205_OFFSET );//fcv205
      // modbusTCPClient7.coilWrite(11, ON); //WP_EN..Control is opposite}
    }
  }
}
...
void reportConfigurabes() {
  ...
  //FCV134_BURNER_FUEL_FLOW_RAMP_BEGIN(fcv134rampbegin), FCV141_SR_FUEL_START_PERCENT(fcv141begin)
...
void fcv205Control() {
  ...
  if (fcv205Override) {
    ...
    if (TT410_HTS_OUT_LREF >= 340) {
      cmdPercentFcv205 = FCV205_PID.calculate(220, TT410_HTS_OUT_LREF);
  ...
  switch (TC_CHECK_COUNTER) {
    ...
    case 3:
      ...
      if (CURRENT_MILLIS - PREVIOUS_MILLIS_3 >= 10) {
        ...
        //     ERROR = 22;
        //  }
        if (TT511_SILICON_CARBIDE_OUT > 1100) {
...
void integrityCheck() {
  ...
  if (!SENSOR_INTEGRITY_CHECK) {
    ...
    modbusTCPClient7.coilWrite(9, OFF); //BLWR_EN ON bc
...
void readOCI() {
  ...
  if (OCI_RESULT == NODE.ku8MBSuccess)
  {
    ...
    Serial.write('_'); Serial.write('!'); Serial.print(F("OCI.INPUT.WORD: ")); Serial.println(OCI_INPUT_STATUS_WORD, BIN);
    ...
    // Serial.write('_'); Serial.write('!'); Serial.print(F("OCI: ")); Serial.println(OCI_RESULT);
...
void readBtn() {
  ...
  if (bitRead(CURRENT_DI_STATUS_WORD, 0)) { //estop
    ...
    modbusTCPClient7.coilWrite(3, OFF); //501
...
void readAI() {
  ...
  FT132_NG_FEED_FLOW = map(modbusTCPClient6.inputRegisterRead(2), 0, 65535, 0, 2.8);
