...
void setup() {
 ...
 // myPort = new Serial(this, portName, 9600);
  ...
  //knobs
  int intx2 = 0, inty2 = 300; //for spacing knob controlers
  for (int i = 36; i < 41; i++) {
    if (i >= 38) {
      cp5.addKnob(myControllers[i])
        .setRange(0, 100)
        .setPosition(1200 + intx2, 1100 + inty2)
        .setRadius(120)
        .setColorForeground(0xffffd800)
  ...
  int intx4 = 3200, inty4 = 1400;//used for override toggles
  ...
  int intx1 = 0, inty1 = 300; //for spacing switch controllers
  int Toggle_offset = 2100;//START OF CHECKBOXES
  for (int i = 41; i <= 49; i++) {
    cp5.addToggle(myControllers[i])
      .setPosition(Toggle_offset + intx1, 1500 + inty1)
      .setColorForeground(0xffffd800)
      .setColorActive(0xffffd800)
      .setSize(188, 63)
      .setValue(0);
      ...
      if (header2 == '+') {
        ...
        if (dataString!=null) {
          ...
          if (dataString.length()<3) {
            cp5.getController(myControllers[49]).setValue(0);//xv909 toggle
            cp5.getController(myControllers[46]).setValue(0);//xv501 toggle
            cp5.getController(myControllers[45]).setValue(0);//xv1100 toggle
            cp5.getController(myControllers[41]).setValue(0);//xv801toggle
            cp5.getController(myControllers[44]).setValue(0);//twv308 toggle
            cp5.getController(myControllers[48]).setValue(0);//twv901 toggle
            cp5.getController(myControllers[47]).setValue(0);//bmm_cr2 toggle/////
            cp5.getController(myControllers[42]).setValue(0);//blwrEntoggle
            cp5.getController(myControllers[43]).setValue(0);//wp_En toggle
          }
          if (dataString.length() == 3) {
