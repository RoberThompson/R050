...
void burnerFuelPID( int x ) {
  if (fcv134Override) {
    ...
    cmdPercentFcv134 = BURNER_FUEL_TEMP_CONTROLLER.calculate(x, TT511_SILICON_CARBIDE_OUT);
...
void superheatTest() {
  ...
  if (wpOverride) {
    ...
    if (abs( TT303_HX504_STEAM_OUT - TT306_EJECTOR_STEAM_IN) >= 40 && TT303_HX504_STEAM_OUT > 200
        || TT306_EJECTOR_STEAM_IN > 200) {
...
void readPTs() {
  ...
  PT420_STEAM_EJECTOR_PRESSURE = map( modbusTCPClient6.inputRegisterRead(5), 0, 65535, 0, 50); // read Channel 5 PT420
...
void readAI() {
  ...
  //FT132_NG_FEED_FLOW = SR_FuelmodbusTCPClient6.inputRegisterRead(2);  
