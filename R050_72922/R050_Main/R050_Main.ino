...
PID BURNER_FUEL_TEMP_CONTROLLER = PID(0.3, 6100, 3700, 40, 0, 0.3);
PID OPEN_SR_FUEL_PID = PID(0.2, 6000, FCV141_SR_FUEL_START_PERCENT, 30, 0, 0);//dt,max,min,kp,kd,ki;//
PID FCV205_PID = PID(0.2, 0, 5200, 5 ,0, 0);
PID monitorSRTubesPID = PID(0.3, 6100, 4300, 80, 0.2, 0.3);
    ...
    switch (FSM_STATE) {
      ...
      case INITIALIZE: //initialize//0
        ...
        if (BMM_PROOF_OF_FLAME) { //get that flame out before proceed
          ...
          modbusTCPClient1.holdingRegisterWrite(42, OFF);//fcv134
      ...
      case OPEN_SR_FUEL:
       ...
       // burnerFuelPID(BURNER_TEMP_CROSSOVER);
        ...
        if (CURRENT_MILLIS - PREVIOUS_MILLIS <= OPEN_SR_FUEL_TIMER) {
          ...
          if ((FT132_NG_FEED_FLOW  > (SR_FUEL_CUT + 3)) || (FT132_NG_FEED_FLOW  < (SR_FUEL_CUT - 3))) {
            PREVIOUS_MILLIS = millis();
            break;
          }
